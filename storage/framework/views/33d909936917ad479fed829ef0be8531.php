...
<body class="bg-gray-100">
    <header class="w-screen border-b-rounded grid grid-cols-2 items-center py-4 lg:py-10 lg:grid-cols-3 bg-gray-800">
        <div class="flex items-center flex-shrink-0 text-white mr-6 lg:pl-10">
            <span class="font-semibold text-xl tracking-tight">Gage's Portfolio</span>
        </div>
        <div class="hidden lg:flex lg:justify-center lg:col-start-2"></div>
        <div class="flex lg:justify-end pr-6 lg:pr-10">
            <?php if(Route::has('login')): ?>
                <?php if(auth()->guard()->check()): ?>
                    <form method="POST" action="<?php echo e(route('logout')); ?>" class="flex">
                        <?php echo csrf_field(); ?>
                        <button type="button" class="text-white mr-0 sm:ml-2 bg-gray-800 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-gray-700 dark:hover:bg-white dark:focus:ring-white dark:hover:text-black">
